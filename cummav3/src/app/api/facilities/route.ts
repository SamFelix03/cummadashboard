  ...
  try {
    ...
    const facilityData: FacilityData = {
      ...
      details: {
        ...
        rentalPlans: Array.isArray(data.details.rentalPlans) ? data.details.rentalPlans.map((plan: RentalPlan) => ({
          name: plan.name,
          price: plan.price,
    ...
    switch (data.facilityType) {
      ...
      case 'prototyping-labs':
        (facilityData.details as LabFacilityDetails).equipment = Array.isArray(data.details.equipment) ? data.details.equipment.map((eq: Equipment) => ({
          labName: eq.labName,
      ...
      case 'raw-space-lab':
        (facilityData.details as RawSpaceFacilityDetails).areaDetails = Array.isArray(data.details.areaDetails) ? data.details.areaDetails.map((area: AreaDetail) => ({
          area: area.area,
          type: area.type,
      ...
      case 'software':
        (facilityData.details as SoftwareFacilityDetails).equipment = Array.isArray(data.details.equipment) ? data.details.equipment.map((eq: any) => ({
      ...
      case 'saas-allied':
        (facilityData.details as SaasFacilityDetails).equipment = Array.isArray(data.details.equipment) ? data.details.equipment.map((eq: any) => ({
          equipmentName: eq.equipmentName,
          capacityAndMake: eq.capacityAndMake
        })) : []
        break
      ...
      case 'individual-cabin':
        (facilityData.details as IndividualCabinDetails).totalCabins = data.details.totalCabins
        ;(facilityData.details as IndividualCabinDetails).availableCabins = data.details.availableCabins
        break

      case 'coworking-spaces':
        (facilityData.details as CoworkingSpaceDetails).totalSeats = data.details.totalSeats
        ;(facilityData.details as CoworkingSpaceDetails).availableSeats = data.details.availableSeats
        break

      case 'meeting-rooms':
        (facilityData.details as MeetingRoomDetails).totalRooms = data.details.totalRooms
        ;(facilityData.details as MeetingRoomDetails).seatingCapacity = data.details.seatingCapacity
        ;(facilityData.details as MeetingRoomDetails).totalTrainingRoomSeaters = data.details.totalTrainingRoomSeaters
        break
    }
